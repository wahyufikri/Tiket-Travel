...
@section('content')
    <div class="container mx-auto p-6">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-2xl font-bold">Data Sopir</h2>
            <a href="/sopir/create" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">
                + Sopir Baru
            </a>
        </div>
        @if (session('success'))
        ...
        <div class="mb-4">
            <form action="{{ url('/sopir') }}" method="GET" class="flex items-center max-w-sm space-x-2">
            ...
            <table class="min-w-full text-sm text-left">
                <thead class="bg-gray-100 text-gray-700 uppercase">
                    <tr>
                        ...
                        <th class="px-4 py-2">
                            <a href="{{ route('sopir.index', array_merge(request()->query(), ['sort_by' => 'name', 'sort_direction' => $sortBy === 'name' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}"
                        ...
                        <th class="px-4 py-2">
                            <a href="{{ route('sopir.index', array_merge(request()->query(), ['sort_by' => 'phone_number', 'sort_direction' => $sortBy === 'phone_number' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}"
                                class="flex items-center space-x-1">
                                <span>No Hp</span>
                        ...
                        <th class="px-4 py-2">
                            <a href="{{ route('sopir.index', array_merge(request()->query(), ['sort_by' => 'address', 'sort_direction' => $sortBy === 'address' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}"
                                class="flex items-center space-x-1">
                                <span>Alamat</span>
                                @if ($sortBy === 'address')
                                    <i class="fas {{ $sortDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down' }}"></i>
                        ...
                        <th class="px-4 py-2">
                            <a href="{{ route('sopir.index', array_merge(request()->query(), ['sort_by' => 'status', 'sort_direction' => $sortBy === 'status' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}"
                                class="flex items-center space-x-1">
                                <span>Status</span>
                                @if ($sortBy === 'status')
                                    <i class="fas {{ $sortDirection === 'asc' ? 'fa-sort-up' : 'fa-sort-down' }}"></i>
                    ...
                    @foreach ($drivers as $index => $driver)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 flex space-x-2">
                                <a href="/sopir/{{ $driver->id }}/edit"
                                    class="text-yellow-500 hover:text-yellow-700">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="/sopir/{{ $driver->id }}" method="post" class="d-inline">
                                    @method('DELETE')
                                    @csrf
                                    <button type="button"
    class="fas fa-trash text-red-500 hover:text-red-700 btn-delete"
    data-id="{{ $driver->id }}"></button>
