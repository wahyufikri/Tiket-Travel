...
@section('content')
    <div class="container mx-auto p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="/manajemen-role/create" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">
                + Role
            </a>
        </div>
        @if (session('success'))
        ...
        @endif
        <div class="mb-4">
            <form action="{{ url('/manajemen-role') }}" method="GET" class="flex items-center max-w-sm space-x-2">
                <div class="relative w-full">
                    ...
                    <input type="text" name="search" placeholder="Cari role..."
            ...
            <table class="min-w-full text-sm text-left">
                <thead class="bg-gray-100 text-gray-700 uppercase">
                    <tr>
                        ...
                        <th class="px-4 py-2">
                            <a href="{{ route('manajemen-role.index', array_merge(request()->query(), ['sort_by' => 'name', 'sort_direction' => $sortBy === 'name' && $sortDirection === 'asc' ? 'desc' : 'asc'])) }}"
                    ...
                    @foreach ($roles as $index => $role)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 flex space-x-2">
                                <a href="/manajemen-role/{{ $role->id }}/edit"
                                    class="text-yellow-500 hover:text-yellow-700">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="/manajemen-role/{{ $role->id }}" method="post" class="d-inline">
                                    @method('DELETE')
                                    @csrf
                                    <button type="button"
    class="fas fa-trash text-red-500 hover:text-red-700 btn-delete"
    data-id="{{ $role->id }}"></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
        ...
        button.addEventListener('click', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `/manajemen-role/${roleId}`;
